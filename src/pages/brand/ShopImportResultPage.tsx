import { Box, Grid, Group, LoadingOverlay, RingProgress, Select, Table, Text, rem, useComputedColorScheme } from "@mantine/core";
...
const ShopImportResultPage = () => {
    ...
    const { data, isLoading } = useGetShopUpsertTaskResult(params?.id ?? "")
    const total = (data?.inserted ?? 0) + (data?.updated ?? 0) + (data?.failed ?? 0)
